...
<th
    ...
    <div class="flex in-[.group\/center-align]:justify-center in-[.group\/end-align]:justify-end"
        ...
        @else
            <button type="button"
                class="group/sortable flex items-center gap-1 -my-1 -ms-2 px-2 py-1 in-[.group\/end-align]:flex-row-reverse in-[.group\/end-align]:-me-2 in-[.group\/end-align]:-ms-8">
                <div>{{ $slot }}</div>

                @if ($sorted)
                    <div
                        @class([
                            "rounded-sm text-zinc-400 group-hover/sortable:text-zinc-800 dark:group-hover/sortable:text-white", "rotate-180" =>
                                $direction == 'desc'
                        ])>
